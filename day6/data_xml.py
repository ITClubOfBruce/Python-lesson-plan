'''
    从xml文件中提取数据
    在java或者Android开发中常见
    html4.01 超文本标记语言   - 2014年10月24日  HTML5
    xhtml  2005年  
    xml

    使用xml.etree.ElementTree 模块从简单的XML文档提取数据
'''
...
'''
    从  https://www.tripadvisor.cn/Attractions-g298557-Activities-oa30-Xi_an_Shaanxi.html#FILTERED_LIST
    获取景点数据
    两个第三方库：beautifulsoup4   解析请求过来的网络数据的
                 requests   网络请求
    一个xml或者html解析库: lxml
'''
import requests
...
# print('--------------------------------------')
# html_obj = etree.HTML(web_data.text)
# 给元素定位的，获取所有的元素 xpath('//p')(获取所有的p标签)
# root.cssselect('button')
...
'''
    爬虫分成四个等级：
        ...
        3. 从网络网站爬取
        4. 从APP爬取，微信是最难的，
        vue，react（SPA single page application 单页应用）
'''
